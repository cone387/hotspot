...
timestamp_pattern = re.compile('\D(1[5-9]\d{8})(\d{3}\D|\D)')
...
date_pattern_list = [(re.compile(x), format_func) for x, format_func in [
    ('(\d{2})[-/年\.](\d{1,2})[-/月\.](\d{1,2})日? ?(\d{1,2}):(\d{1,2})', get_ymdhm_date),   # 年月日 时分
    ('\D(\d{1,2})[-/月](\d{1,2})日* *(\d{1,2}):(\d{1,2})', get_mdhm_date),    # 月日  时分
    ('(\d{2})[-/年](\d{1,2})[-/月](\d{1,2})日?', get_ymd_date),  # 年月日
    ('(\d{1,2}):(\d{1,2})', get_hm_date),  # 时分
    ('(\d{1,2})[-/月](\d{1,2})日?', get_md_date),    # 月日
]]

time_pattern_list = [(re.compile(x), format_func) for x, format_func in [
    ('(半|\d{1,2}) ?小时前', get_hour_timestamp),  # n小时前
    ('(\d{1,2}) ?分钟*前', get_minute_timestamp),    # n分钟前
    ('(刚刚|\d{1,2}) ?秒前', get_second_timestamp),
    ('\D(1[5-9]\d{8})(\d{3}\D|\D)', get_int_timestamp)
]]
